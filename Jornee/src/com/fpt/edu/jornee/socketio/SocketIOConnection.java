 ...
 * FileName: SocketIOConnection.java
...
import com.fpt.edu.jornee.socketio.SocketIOMessage;
import com.fpt.edu.jornee.socketio.SocketIOOptions;
import com.fpt.edu.jornee.socketio.SocketIOReader;
import com.fpt.edu.jornee.socketio.SocketIOWriter;
...
public class SocketIOConnection extends WebSocketConnection implements SocketIO {
    ...
    public void connect(String wsUri, SocketIO.ConnectionHandler sessionHandler, SocketIOOptions options) {
       ...
       new SocketIOConnector(wsUri, sessionHandler, options, null).execute();
    }
    
    @Override
    public void connectCustom(String wsUri, SocketIO.ConnectionHandler sessionHandler, String token) {
        SocketIOOptions options = new SocketIOOptions();
        options.setReceiveTextMessagesRaw(true);
        options.setMaxMessagePayloadSize(64*1024);
        options.setMaxFramePayloadSize(64*1024);
        ...
        connectCustom(wsUri, sessionHandler, options, token);
    }
     ...
     */
    @Override
    public void connectCustom(String wsUri, SocketIO.ConnectionHandler sessionHandler, SocketIOOptions options, String token) {

       mSessionHandler = sessionHandler;
       mEvents.clear();
       
       new SocketIOConnector(wsUri, sessionHandler, options, token).execute();
    }
    ...
    private class SocketIOConnector extends AsyncTask<Void, Void, String> {
        ...
        public SocketIOConnector(String wsUri, SocketIO.ConnectionHandler sessionHandler, SocketIOOptions options, String token){
            super();
            this.wsUri = wsUri;
        ...
        protected String doInBackground(Void... params) {
            ...
            try {
                HttpPost post = new HttpPost("http"+wsUri.substring(2)+"socket.io/1/?token="+token);
